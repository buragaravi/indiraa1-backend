 ...
 * @param {Array} products - Array of product objects
 * @param {Array} imageFiles - Array of image files
 * @returns {Object} - Object with matchedProducts and unmatchedImages
 */
export const matchImagesToProducts = (products, imageFiles) => {
  // Handle edge cases
  if (!products || !Array.isArray(products)) {
    console.warn('[IMAGE MATCHING] Invalid products array provided');
    return { matchedProducts: [], unmatchedImages: imageFiles || [] };
  }

  if (!imageFiles || !Array.isArray(imageFiles) || imageFiles.length === 0) {
    console.log('[IMAGE MATCHING] No image files provided, returning products without images');
    return { 
  ...
  console.log(`[IMAGE MATCHING] Processing ${products.length} products with ${imageFiles.length} images`);

  products.forEach((product, index) => {
    if (!product || typeof product !== 'object' || !product.name) {
      console.warn(`[IMAGE MATCHING] Invalid product at index ${index}:`, product);
    ...
    const mainProductImages = imageFiles.filter(file => {
      ...
      // Check if image belongs to this product but not to any specific variant
      const belongsToProduct = (
        filename.includes(productName) ||
        filename.includes(product.name.toLowerCase())
      );
      
      // Check if it's NOT a variant-specific image
      const isVariantSpecific = product.variants?.some(variant => {
        if (!variant || !variant.name) return false;
        const variantName = variant.name.toLowerCase().replace(/\s+/g, '_');
        return filename.includes(variantName) || filename.includes(variant.name.toLowerCase());
      });
      
      return belongsToProduct && !isVariantSpecific;
    });
    ...
    const productResult = {
      ...
      variantImages: variantMatches, // For backward compatibility
      variantMatches: variantMatches  // Standard naming
    };

    matchedProducts.push(productResult);

    console.log(`[IMAGE MATCHING] Product "${product.name}": ${productMatches.length} main images, ${Object.keys(variantMatches).length} variant groups`);
  });

  // Find unmatched images
  const allMatchedFiles = new Set();
  matchedProducts.forEach(product => {
    if (product.matchedImages && Array.isArray(product.matchedImages)) {
      product.matchedImages.forEach(file => {
        if (file && file.originalname) {
          allMatchedFiles.add(file.originalname);
        }
      });
    }
    
    if (product.variantMatches && typeof product.variantMatches === 'object') {
  ...
  imageFiles.forEach(file => {
    if (file && file.originalname && !allMatchedFiles.has(file.originalname)) {
      unmatchedImages.push(file);
    }
  });

  console.log(`[IMAGE MATCHING] Result: ${matchedProducts.length} products processed, ${unmatchedImages.length} unmatched images`);

  return { matchedProducts, unmatchedImages };
};
...
export { s3, BUCKET }; // Export S3 client and bucket for external use